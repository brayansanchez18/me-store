    ...
    <?php if (!empty($carts)): $totalCart = 0 ?>
      <div class="card-body" id="bodyCart">
        <?php foreach ($carts as $key => $value): ?>
          <?php if ($key > 0): ?>
            <hr class="d-block d-lg-none my-4 mx-auto w-75 hr_<?= $key ?>" style="border:1px solid #aaa">
          <?php endif ?>
          ...
          <div class="row py-1" style="position:relative;">
              ...
              <a href="/<?= $value->url_product ?>">
                <div class="media">
                  ...
                  <div class="media-body">
                    <p class="m-0 font-weight-bold"><?= $value->name_product ?> </p>
              ...
              <?php if ($value->type_variant == 'gallery'): ?>
                <div class="blockQuantity d-flex justify-content-center">
                  <div class="input-group mb-3 mt-2" style="width:130px">
                    <span class="input-group-text btnInc" type="btnMin" key="<?= $key ?>" idCart="<?= $value->id_cart ?>">
                      <i class="fas fa-minus"></i>
                    </span>

                    <input type="number" class="form-control text-center showQuantity_<?= $key ?> showQuantity" onwheel="return false;" value="<?= $value->quantity_cart ?>">

                    <span class="input-group-text btnInc" type="btnMax" key="<?= $key ?>" idCart="<?= $value->id_cart ?>">
                      <i class="fas fa-plus"></i>
                    </span>
                  </div>
                </div>
              <?php else: ?>
                <input type="hidden" class="form-control text-center showQuantity" value="<?= $value->quantity_cart ?>">
              <?php endif ?>
            </div>
            ...
            <div class="d-none d-lg-block col-lg-1 text-center mt-3">
              <button class="btn btn-light btn-sm border remCart" key="<?= $key ?>" idCart="<?= $value->id_cart ?>"><i class="fas fa-times"></i></button>
            </div>

            <div class="d-block d-lg-none">
              <button class="btn btn-light btn-sm border remCart" key="<?= $key ?>" idCart="<?= $value->id_cart ?>" style="position:absolute; top:0px; right:0px"><i class="fas fa-times"></i></button>
            </div>
          </div>
...
<script src="<?= $path ?>views/assets/js/carts/carts.js"></script>