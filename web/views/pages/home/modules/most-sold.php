...
<div class="container-fluid bg-white">
  <div class="container">
    ...
    <!-- GRID -->
    <div class="row row-cols-2 row-cols-sm-2 row-cols-md-4 pt-3 pb-4 grid-3">
          ...
          <a href="/<?= $value->url_product ?>">
            <figure class="imgProduct">
              <?php if ($value->type_variant == 'gallery'): ?>
                <img src="<?= $path ?>views/assets/img/products/<?= $value->url_product ?>/<?= json_decode($value->media_variant)[0] ?>" alt="<?= $value->name_product ?>" class="img-fluid">
              <?php else: $arrayYT = explode('/', $value->media_variant) ?>
                <img src="http://img.youtube.com/vi/<?= end($arrayYT) ?>/maxresdefault.jpg" class="img-fluid bg-light" alt="<?= $value->name_product ?>">
              <?php endif ?>
            </figure>
            ...
            <h5 class="float-start text-uppercase text-muted">
              <?php if ($value->offer_variant > 0): ?>
                <del class="small" style="color:#bbb">
                  MXN$<?php echo $value->price_variant ?>
      ...
      <?php foreach ($salesProducts as $key => $value): ?>
        <div class="media border-bottom px-3 pt-4 pb-3 pb-lg-2">
          <a href="/<?= $value->url_product ?>">
            <figure class="imgProduct">
              <?php if ($value->type_variant == 'gallery'): ?>
                <img src="<?= $path ?>views/assets/img/products/<?= $value->url_product ?>/<?= json_decode($value->media_variant)[0] ?>" class="img-fluid" alt="<?= $value->name_product ?>" style="width:150px">
              <?php else: $arrayYT = explode("/", $value->media_variant) ?>
                <img src="http://img.youtube.com/vi/<?= end($arrayYT) ?>/maxresdefault.jpg" class="img-fluid bg-light" alt="<?= $value->name_product ?>" style="width:150px">
              <?php endif ?>
            </figure>
          </a>
