...
if ($(".getWarranty").length > 0) {
  ...
  getWarranty.each((i) => {
    $(getWarranty[i]).attr(
      "href",
      "https://wa.me/" +
        $(getWarranty[i]).attr("phone") +
        "?text=" +
        encodeURIComponent(
          "¡Hola! Quiero iniciar una garantía de la órden de compra N. " +
            $(getWarranty[i]).attr("order")
        )
    );
  });
}
...
if ($(".getRefund").length > 0) {
  ...
  getRefund.each((i) => {
    $(getRefund[i]).attr(
      "href",
      "https://wa.me/" +
        $(getRefund[i]).attr("phone") +
        "?text=" +
        encodeURIComponent(
          "¡Hola! Quiero un reembolso de dinero de la órden de compra N. " +
            $(getRefund[i]).attr("order")
        )
    );
  });
}
...
if ($(".questionOrder").length > 0) {
  ...
  questionOrder.each((i) => {
    $(questionOrder[i]).attr(
      "href",
      "https://wa.me/" +
        $(questionOrder[i]).attr("phone") +
        "?text=" +
        encodeURIComponent(
          "¡Hola! Tengo dudas acerca de la órden de compra N. " +
            $(questionOrder[i]).attr("order")
        )
    );
  });
}
